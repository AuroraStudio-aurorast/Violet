...
class CVePlayList : public CVeListTemplate
{
private:
	...
	void LVPaintSubItem(const D2D1_RECT_F& rcPaint, NMLTCUSTOMDRAW& nm, LRESULT r) override
	{
		...
		if (e.vSubItem.size() == 4) {
			...
			D2D1_RECT_F rcText{};
			D2D1_RECT_F rcItm{};
			GetItemRect(nm.idx, rcItm);
			if (nm.idxSub == 0) {
				rcText = D2D1::RectF(imgWidth + rcItm.left + Padding2 * 3, nm.rc.top + Padding, rcItm.right / 2, nm.rc.top + m_cyItem / 2);
			}
			if (nm.idxSub == 1) {
				rcText = D2D1::RectF(imgWidth + rcItm.left + Padding2 * 3, nm.rc.top + m_cyItem / 2, rcItm.right / 2, nm.rc.top + m_cyItem - Padding);
				textAlpha = 0.5;
			}
			if (nm.idxSub == 2) {
				rcText = D2D1::RectF(rcItm.right / 2, nm.rc.top, rcItm.right - 55, nm.rc.bottom);
			}
			if (nm.idxSub == 3) {
				rcText = D2D1::RectF(rcItm.right - 55, nm.rc.top, rcItm.right, nm.rc.bottom);
			}
#ifdef _DEBUG
			m_pBrush->SetColor(D2D1::ColorF(0xFF0000));
			m_pDC->DrawRectangle(rcText, m_pBrush, 1.0f);
#endif
			...
			if (!pTl.Get() && es.Item.pszText && es.Item.cchText > 0)
			{
				...
				eck::g_pDwFactory->CreateTextLayout(es.Item.pszText, es.Item.cchText,
					GetTextFormat(),
					rcText.right - rcText.left,//nm.rc.right - nm.rc.left - Padding * 3 - sizeImg.width,
			...
			if (pTl.Get())
			{
				...
				{
					m_pBrush->SetColor(D2D1::ColorF(foregroundColor, textAlpha));
					m_pDC->DrawTextLayout({ rcText.left, rcText.top }, pTl.Get(), m_pBrush,
