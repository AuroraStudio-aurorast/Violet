...
float CVeLrc::ItmPaint(int idx)
{
    ...
    m_pRenderer->LrItmDraw(Opt, m_idxCurrAnItem);
    ...
    case WM_SIZE:
    {
        ...
        if (!IsEmpty())
        {
            ...
            else
            {
                ...
                m_pRenderer->LrSetCurrentItemRect(D2D1::RectF(CurrItem.x, CurrItem.y, CurrItem.x + CurrItem.cx, CurrItem.y + CurrItem.cy));
                m_pRenderer->LrUpdateFadeBrush();
...
HRESULT CVeLrc::LrcSetCurrentLine(int idxCurr)
{
    ...
    m_idxCurr = idxCurr;
    const auto idxCurr_ = m_idxCurr < 0 ? 0 : m_idxCurr;
    const auto& CurrItem = m_vItem[idxCurr_];
    m_pRenderer->LrSetCurrentItemRect(D2D1::RectF(CurrItem.x, CurrItem.y, CurrItem.x + CurrItem.cx, CurrItem.y + CurrItem.cy));
    m_pRenderer->LrUpdateFadeBrush();
