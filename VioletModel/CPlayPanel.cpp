...
LRESULT CPlayPanel::OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if (eck::IntersectRect(rcTopSample, rcTop, ps.rcfClipInElem))
		{
			...
			BlurD2dDC(m_pDC, eck::g_pD2DFactory, rcSampleInBmp,
				{ rcTopSample.left,rcTopSample.top }, 15 * m_fade, 10);
		}

		m_pBrush->SetColor(D2D1::ColorF(backgroundColor, 0.15 * m_fade));
		m_pDC->FillRoundedRectangle(D2D1::RoundedRect(ps.rcfClipInElem, 10, 10), m_pBrush);
		m_pBrush->SetColor(D2D1::ColorF(foregroundColor, 0.15 * m_fade));
		m_pDC->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(ps.rcfClipInElem.left + 0.5, ps.rcfClipInElem.top + 0.5, ps.rcfClipInElem.right - 0.5, ps.rcfClipInElem.bottom - 0.5), 10, 10), m_pBrush, 1.f);
		EndPaint(ps);
	}
	return 0;
	...
	case WM_CREATE:
	{
		...
		App->GetFontFactory().NewFont(pTfTitle.RefOf(), eck::Align::Near,
			eck::Align::Center, (float)CyFontPanelTitle, 700);
		App->GetFontFactory().NewFont(pTfSubtitle.RefOf(), eck::Align::Near,
			eck::Align::Center, (float)CyFontPanelSubtitle);
		...
		//float y = DTopTitle;
		float y = DTopMiniCover + (CxyMiniCover - CyPlayPanelText * 2 - CyPaddingTitleAndArtist) / 2;
		m_LATitle.Create(L"Violet", Dui::DES_VISIBLE, 0,
			x, y, CxMaxTitleAndArtist, CyPlayPanelText, this, pWnd);
		m_LATitle.SetColor(D2D1::ColorF(foregroundColor));
		...
		m_LAArtist.Create(L"Player", Dui::DES_VISIBLE, 0,
			x, y, CxMaxTitleAndArtist, CyPlayPanelText, this, pWnd);
		m_LAArtist.SetColor(D2D1::ColorF(foregroundColor, 0.6));
		...
		m_LATime.Create(L"00:00/00:00", Dui::DES_VISIBLE, 0,
			x, DTopTime - CxyPlayPanelPadding / 2, CxMaxTime, CyPlayPanelText, this, pWnd);
		m_LATime.SetColor(D2D1::ColorF(foregroundColor, 0.6));
		m_LATime.SetUserColor(TRUE);
	}
	break;
