...
protected:
    ...
    BOOLEAN m_bTopBtmFade{true};		// 指示是否启用顶部和底部渐变
public:
    ...
    virtual void LrItmDraw(const LRD_DRAW& Opt, int curr) = 0;
    virtual HRESULT LrUpdateEmptyText(const LRD_EMTRY_TEXT& Opt) = 0;
    virtual void LrSetViewSize(float cx, float cy) = 0;
    virtual void LrUpdateFadeBrush() = 0;
    virtual void LrSetCurrentItemRect(D2D1_RECT_F rectCurr) = 0;
