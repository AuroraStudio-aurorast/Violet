...
class CVeEdit;
class CVeEditTextHost : public ITextHost2
{
private:
    ULONG m_cRef{ 1 };
    BITBOOL m_bActive : 1{};
    CVeEdit* m_pEdit{};
public:
    CVeEditTextHost(CVeEdit* pEdit) :m_pEdit{ pEdit } {}
    ...
    STDMETHODIMP QueryInterface(REFIID riid, void** ppvObject) override
    {
        ...
        {
            { &eck::IID_ITextHost, OFFSETOFCLASS(CVeEditTextHost, ITextHost) },
            { &eck::IID_ITextHost2, OFFSETOFCLASS(CVeEditTextHost, ITextHost2) },
            {}
        };
...
struct NMEDTXNOTIFY : Dui::DUINMHDR
{
    int iNotify;
    void* pData;
};

class CVeEdit :public Dui::CElem
{
    friend class CVeEditTextHost;
public:
...
private:
    ...
    eck::Align m_eSingleLineAlignV{ eck::Align::Center };
    ...
    void UpdateInsetRect()
    {
        ...
        {
            if (m_eSingleLineAlignV == eck::Align::Near)
                goto Normal;
            ...
            if (m_eSingleLineAlignV == eck::Align::Center)
            {
...
public:
    LRESULT OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam) override
    {
        ...
        case WM_PAINT:
        {
            ...
            if (m_blurBkg) {
                ...
                if (eck::IntersectRect(rcTopSample, rcTop, ps.rcfClipInElem))
                {
                    ...
                    BlurD2dDC(m_pDC, eck::g_pD2DFactory, rcSampleInBmp,
                        { rcTopSample.left,rcTopSample.top }, 10, 8);
                }
            ...
            if (eState == Dui::State::Normal) {
                pBrush->SetColor(D2D1::ColorF(backgroundColor, 0.5));
                m_pDC->FillRoundedRectangle(D2D1::RoundedRect(rcViewF, 8, 8), pBrush);
                pBrush->SetColor(D2D1::ColorF(foregroundColor, 0.15));
                m_pDC->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(rcViewF.left + 0.5, rcViewF.top + 0.5, rcViewF.right - 0.5, rcViewF.bottom - 0.5), 8, 8), pBrush, 1.0f);
            }
            if (eState == Dui::State::Hot) {
                pBrush->SetColor(D2D1::ColorF(backgroundColor, 0.8));
                m_pDC->FillRoundedRectangle(D2D1::RoundedRect(rcViewF, 8, 8), pBrush);
                pBrush->SetColor(D2D1::ColorF(foregroundColor, 0.3));
                m_pDC->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(rcViewF.left + 0.5, rcViewF.top + 0.5, rcViewF.right - 0.5, rcViewF.bottom - 0.5), 8, 8), pBrush, 1.0f);
            }
            if (eState == Dui::State::Focused) {
                pBrush->SetColor(D2D1::ColorF(backgroundColor, 1));
                m_pDC->FillRoundedRectangle(D2D1::RoundedRect(rcViewF, 8, 8), pBrush);
                pBrush->SetColor(D2D1::ColorF(highlightColor, 1));
                m_pDC->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(rcViewF.left + 0.5, rcViewF.top + 0.5, rcViewF.right - 0.5, rcViewF.bottom - 0.5), 8, 8), pBrush, 1.0f);
            }
            BSTR TxText;
            m_pSrv->TxGetText(&TxText);
            if (eState != Dui::State::Focused && TxText == NULL) {
                pBrush->SetColor(D2D1::ColorF(foregroundColor, 0.5));
                OYM_CvsDrawText(m_pDC, m_CueBanner, L"Segoe UI", CyFontNormal, false, pBrush, D2D1::RectF(rcViewF.left + 8, rcViewF.top, rcViewF.right, rcViewF.bottom), DWRITE_PARAGRAPH_ALIGNMENT_CENTER, DWRITE_TEXT_ALIGNMENT_LEADING, false);
            }
        ...
        case WM_SIZE:
        {
            const auto cxSB = GetTheme()->GetMetrics(Dui::Metrics::CxVScroll);
            const auto cySB = GetTheme()->GetMetrics(Dui::Metrics::CyHScroll);
        ...
        case WM_CREATE:
        {
            ...
            m_SBV.GetScrollView()->SetCallBack([](float fPos, float fPrevPos, LPARAM lParam)
                {
                    const auto pThis = (CVeEdit*)lParam;
            ...
            m_pSrv->OnTxUIActivate();
            if (GetWnd()->GetUserDpi() != eck::GetDpi(nullptr))
                OnDpiChanged();
        }
        break;
        ...
        case WM_DESTROY:
        {
            ...
            eck::TsiShutdownTextServices(m_pSrv);
            m_pSrv = nullptr;
            SafeRelease(pBrush);
            SafeRelease(m_pHost);
        }
        ...
        case WM_NCCREATE:
        {
            if (!eck::TsiIsAvailable())
                eck::TsiInit();
            m_pHost = new CVeEditTextHost{ this };
            IUnknown* pUnk{};
            eck::TsiCreateTextServices(nullptr, m_pHost, &pUnk);
            pUnk->QueryInterface(eck::IID_ITextServices2, (void**)&m_pSrv);
            pUnk->Release();
        }
        break;
        }
    ...
    void Scroll(BOOL bHorz, int nPos, BOOL bRedraw = TRUE)
    {
        POINT pt;
        GetScrollPos(&pt);
        (bHorz ? pt.x : pt.y) = eck::DpiScale(nPos, GetWnd()->GetDpiValue(), 96);
        SetScrollPos(&pt);
        if (bRedraw)
            InvalidateRect();
    }
    ...
    void setCueBanner(LPWSTR CueBanner) {
        m_CueBanner = CueBanner;
    }
...
inline HDC CVeEditTextHost::TxGetDC()
{
    return GetDC(m_pEdit->GetWnd()->HWnd);
}

inline INT CVeEditTextHost::TxReleaseDC(HDC hdc)
{
    return ReleaseDC(m_pEdit->GetWnd()->HWnd, hdc);
}

inline BOOL CVeEditTextHost::TxShowScrollBar(INT fnBar, BOOL fShow)
{
    m_pEdit->ShowScrollBar(fnBar, fShow);
    return TRUE;
}

inline BOOL CVeEditTextHost::TxEnableScrollBar(INT fuSBFlags, INT fuArrowflags)
{
    // TODO
    return 0;
}

inline BOOL CVeEditTextHost::TxSetScrollRange(INT fnBar, LONG nMinPos, INT nMaxPos, BOOL fRedraw)
{
    eck::CCsGuard _{ m_pEdit->GetCriticalSection() };
    const auto fMin = eck::DpiScaleF((float)nMinPos, 96, m_pEdit->GetWnd()->GetDpiValue());
    const auto fMax = eck::DpiScaleF((float)nMaxPos, 96, m_pEdit->GetWnd()->GetDpiValue());
...
inline BOOL CVeEditTextHost::TxSetScrollPos(INT fnBar, INT nPos, BOOL fRedraw)
{
    eck::CCsGuard _{ m_pEdit->GetCriticalSection() };
...
inline void CVeEditTextHost::TxInvalidateRect(LPCRECT prc, BOOL fMode)
{
    if (prc)
    {
        D2D1_RECT_F rc{ eck::MakeD2DRectF(*prc) };
...
inline BOOL CVeEditTextHost::TxCreateCaret(HBITMAP hbmp, INT xWidth, INT yHeight)
{
    m_pEdit->m_rcCaret = { 0,0,(float)xWidth,(float)yHeight };
    return TRUE;
}

inline BOOL CVeEditTextHost::TxShowCaret(BOOL fShow)
{
...
inline BOOL CVeEditTextHost::TxSetCaretPos(INT x, INT y)
{
    auto& rc = m_pEdit->m_rcCaret;
...
inline BOOL CVeEditTextHost::TxSetTimer(UINT idTimer, UINT uTimeout)
{
    return m_pEdit->SetTimer(idTimer, uTimeout);
}

inline void CVeEditTextHost::TxKillTimer(UINT idTimer)
{
    m_pEdit->KillTimer(idTimer);
}

inline void CVeEditTextHost::TxSetCapture(BOOL fCapture)
{
    if (fCapture)
        m_pEdit->SetCapture();
    else
...
inline void CVeEditTextHost::TxSetFocus()
{
    m_pEdit->SetFocus();
}

inline void CVeEditTextHost::TxSetCursor(HCURSOR hcur, BOOL fText)
{
    SetCursor(hcur);
}

inline BOOL CVeEditTextHost::TxScreenToClient(LPPOINT lppt)
{
    return ScreenToClient(m_pEdit->GetWnd()->HWnd, lppt);
}

inline BOOL CVeEditTextHost::TxClientToScreen(LPPOINT lppt)
{
    return ClientToScreen(m_pEdit->GetWnd()->HWnd, lppt);
}

inline HRESULT CVeEditTextHost::TxActivate(LONG* plOldState)
{
...
inline HRESULT CVeEditTextHost::TxDeactivate(LONG lNewState)
{
    m_bActive = lNewState;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetClientRect(LPRECT prc)
{
    auto rcF{ m_pEdit->GetRectInClientF() };
    m_pEdit->GetWnd()->Log2Phy(rcF);
    *prc = eck::MakeRect(rcF);
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetViewInset(LPRECT prc)
{
    *prc = m_pEdit->m_rcViewInset;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetCharFormat(const CHARFORMATW** ppCF)
{
    *ppCF = &m_pEdit->m_DefCharFormat;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetParaFormat(const PARAFORMAT** ppPF)
{
    *ppPF = &m_pEdit->m_DefParaFormat;
    return S_OK;
}

inline COLORREF CVeEditTextHost::TxGetSysColor(int nIndex)
{
    D2D1_COLOR_F cr;
    switch (nIndex)
    {
    case COLOR_WINDOWTEXT:
        m_pEdit->GetTheme()->GetSysColor(Dui::SysColor::Text, cr);
        return eck::D2DColorFToColorref(cr);
    case COLOR_WINDOW:
        m_pEdit->GetTheme()->GetSysColor(Dui::SysColor::Bk, cr);
        return eck::D2DColorFToColorref(cr);
    default:
...
inline HRESULT CVeEditTextHost::TxGetBackStyle(TXTBACKSTYLE* pstyle)
{
    *pstyle = TXTBACK_TRANSPARENT;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetMaxLength(DWORD* plength)
{
    *plength = INFINITE;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetScrollBars(DWORD* pdwScrollBar)
{
    *pdwScrollBar = WS_VSCROLL | WS_HSCROLL | ES_AUTOVSCROLL | ES_AUTOHSCROLL;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetPasswordChar(_Out_ TCHAR* pch)
{
    *pch = m_pEdit->m_chPassword;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxGetAcceleratorPos(LONG* pcp)
{
    *pcp = -1;
    return E_NOTIMPL;
}

inline HRESULT CVeEditTextHost::TxGetExtent(LPSIZEL lpExtent)
{
...
inline HRESULT CVeEditTextHost::OnTxCharFormatChange(const CHARFORMATW* pCF)
{
    return E_NOTIMPL;
}

inline HRESULT CVeEditTextHost::OnTxParaFormatChange(const PARAFORMAT* pPF)
{
    return E_NOTIMPL;
}

inline HRESULT CVeEditTextHost::TxGetPropertyBits(DWORD dwMask, DWORD* pdwBits)
{
    *pdwBits = (m_pEdit->TxGetProp() | TXTBIT_D2DDWRITE) & dwMask;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxNotify(DWORD iNotify, void* pv)
{
    NMEDTXNOTIFY nm{ Dui::EDE_TXNOTIFY };
    nm.iNotify = iNotify;
    nm.pData = pv;
...
inline HIMC CVeEditTextHost::TxImmGetContext()
{
    return ImmGetContext(m_pEdit->GetWnd()->HWnd);
}

inline void CVeEditTextHost::TxImmReleaseContext(HIMC himc)
{
    ImmReleaseContext(m_pEdit->GetWnd()->HWnd, himc);
}

inline BOOL CVeEditTextHost::TxIsDoubleClickPending()
{
    MSG msg;
...
inline HRESULT CVeEditTextHost::TxGetWindow(HWND* phwnd)
{
    *phwnd = m_pEdit->GetWnd()->HWnd;
    return S_OK;
}

inline HRESULT CVeEditTextHost::TxSetForegroundWindow()
{
...
inline HRESULT CVeEditTextHost::TxGetEastAsianFlags(LONG* pFlags)
{
    *pFlags = 0;
    return S_OK;
}

inline HCURSOR CVeEditTextHost::TxSetCursor2(HCURSOR hcur, BOOL bText)
{
    return SetCursor(hcur);
}

inline HRESULT CVeEditTextHost::TxGetWindowStyles(DWORD* pdwStyle, DWORD* pdwExStyle)
{
...
inline HRESULT CVeEditTextHost::TxShowDropCaret(BOOL fShow, HDC hdc, LPCRECT prc)
{
    return E_NOTIMPL;
}

inline HRESULT CVeEditTextHost::TxDestroyCaret()
{
    return E_NOTIMPL;
}

inline HRESULT CVeEditTextHost::TxGetHorzExtent(LONG* plHorzExtent)
{
    return E_NOTIMPL;
}