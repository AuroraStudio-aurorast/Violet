...
LRESULT CVeMiniCover::OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		RoundedRectMaskD2dDC(m_pDC, eck::g_pD2DFactory, rcView, 10);
		...
		BlurDC:
			...
			m_pDC->DrawBitmap(m_pBmp, rcView, 1.0f, D2D1_INTERPOLATION_MODE_HIGH_QUALITY_CUBIC);
			m_pDC->Flush();
			...
			m_BgBrush->SetColor(D2D1::ColorF(backgroundColor, 0.07 * fValue));
			m_pDC->FillRoundedRectangle(D2D1::RoundedRect(rcView, 0, 0), m_BgBrush);
		...
		else
		{
			if (m_bHover)
			{
				...
				m_pDC->DrawBitmap(m_pBmp, GetViewRectF(), 1.0f, D2D1_INTERPOLATION_MODE_HIGH_QUALITY_CUBIC);
		}
		m_pDC->PopLayer();
