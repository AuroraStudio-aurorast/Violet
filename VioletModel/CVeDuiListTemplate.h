...
#include "CVeDuiHeader.h"
#include "eck/CD2DImageList.h"


struct NMLTITEMINDEX : eck::Dui::DUINMHDR
{
	int idx;
	int idxGroup;
};
...
struct NMLTSCROLLED : eck::Dui::DUINMHDR
{
	int idxBegin;
	int idxGroupBegin;
	int idxEnd;
...
struct NMLTCUSTOMDRAW : eck::Dui::NMECUSTOMDRAW
{
...
class CVeListTemplate :public eck::Dui::CElem
{
public:
...
protected:
	...
	CVeScrollBar m_SBV{}, m_SBH{};
	CVeHeader m_Header{};
	ID2D1SolidColorBrush* m_pBrush{};
	eck::CInertialScrollView* m_psvV{};
	eck::CInertialScrollView* m_psvH{};
	eck::CD2DImageList * m_pImgList{};
	eck::CD2DImageList* m_pImgListGroup{};
			...
			if (!(r & CDRF_SKIPDEFAULT))
			{
				if (nm.eState != eck::Dui::State::None) {
					...
					if (nm.eState == eck::Dui::State::Selected) {
						m_pBrush->SetColor(D2D1::ColorF(highlightColor, 0.1));
						m_pDC->FillRoundedRectangle(D2D1::RoundedRect(nm.rc, 8, 8), m_pBrush);
					}
					...
					//GetTheme()->DrawBackground(eck::Dui::Part::ListItem, nm.eState, nm.rc, nullptr);
	...
	virtual void PrePaint(eck::Dui::ELEMPAINTSTRU& ps) {}
	virtual void PostPaint(eck::Dui::ELEMPAINTSTRU& ps) {}
	...
	void GRDragSelMouseMove(POINT pt, WPARAM wParam)
	{
		...
		eck::OffsetRect(rcOld, 0, -dy);

		m_rcDragSel = eck::MakeD2DRectF(eck::MakeRect(pt, POINT{
			m_ptDragSelStart.x,
		...
		D2D1_RECT_F rcItem;
		D2D1_RECT_F rcJudge;
		eck::UnionRect(rcJudge, rcOld, m_rcDragSel);
		if (eck::IsRectEmpty(rcJudge))
			goto Skip;
		...
		for (int i = idxGroupBegin; i <= idxGroupEnd; ++i)
		{
			...
			for (int j = j0; j <= j1; ++j)
			{
				...
				const BOOL bIntersectOld = eck::IsRectsIntersect(rcItem, rcOld);
				const BOOL bIntersectNew = eck::IsRectsIntersect(rcItem, m_rcDragSel);
	...
	Skip:
		eck::OffsetRect(m_rcDragSel, 0.f, dy);
		InvalidateRect();
	}
	...
	void OnPaint(WPARAM wParam, LPARAM lParam)
	{
		...
		PrePaint(ps);
		NMLTCUSTOMDRAW nm{};
		nm.uCode = eck::Dui::EE_CUSTOMDRAW;
	...
	SkipDef:
		...
		{
			auto rc{ m_rcDragSel };
			eck::OffsetRect(rc, 0.f, (float)-m_psvV->GetPos());
			//GetTheme()->DrawBackground(eck::Dui::Part::ListSelRect, eck::Dui::State::None, rc, nullptr);

			m_pBrush->SetColor(D2D1::ColorF(highlightColor, 0.1));
			m_pDC->FillRoundedRectangle(D2D1::RoundedRect(rc, 8, 8), m_pBrush);
			m_pBrush->SetColor(D2D1::ColorF(highlightColor, 0.2));
			m_pDC->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(rc.left + 0.5, rc.top + 0.5, rc.right - 0.5, rc.bottom - 0.5), 8, 8), m_pBrush);
	...
	void OnVScroll()
	{
		ReCalcTopItem();
		InvalidateRect();
		NMLTSCROLLED nm{ eck::Dui::LTE_SCROLLED };
...
public:
	LRESULT OnEvent(UINT uMsg, WPARAM wParam, LPARAM lParam) override
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			int idx = HitTest(ht);
			NMLTHOTITEMCHANGE nm{ eck::Dui::LTE_HOTITEMCHANED };
			if (m_bGroup)
			{
		...
		case WM_NOTIFY:
		{
				...
				(((eck::Dui::DUINMHDR*)lParam)->uCode == eck::Dui::EE_VSCROLL))
			{
				...
				(((eck::Dui::DUINMHDR*)lParam)->uCode == eck::Dui::EE_HSCROLL))
			{
				...
				switch (((eck::Dui::DUINMHDR*)lParam)->uCode)
				{
				case eck::Dui::HEE_GETDISPINFO:
					return GenElemNotify((eck::Dui::NMHEDISPINFO*)lParam);
				case eck::Dui::HEE_WIDTHCHANGED:
				{
					EckAssert(m_eView == Type::Report);
					const auto* const p = (eck::Dui::NMHEITEMNOTIFY*)lParam;
					ArrangeHeader(TRUE);
					ReCalcHScroll();
					D2D1_RECT_F rc;
		...
		case WM_LBUTTONDOWN:
		{
			...
			if (idx >= 0)
			{
				...
				if (bDid)
				{
					...
					eck::UnionRect(rcInvalid, rcInvalid, rcItem);
				}
			...
			else
			{
				...
				if (!m_bSingleSel && m_bEnableDragSel &&
					eck::IsMouseMovedBeforeDragging(GetWnd()->HWnd, pt.x, pt.y))
				{
					if (!GetWnd()->IsValid())
						return 0;
					SetCapture();
		...
		case WM_CREATE:
		{
			...
			m_SBV.Create(nullptr, eck::Dui::DES_VISIBLE, 0,
				0, 0, 0, 0, this);
			m_psvV = m_SBV.GetScrollView();
			m_psvV->AddRef();
			m_psvV->SetMinThumbSize(eck::Dui::CxyMinScrollThumb);
			m_psvV->SetCallBack([](float fPos, float fPrevPos, LPARAM lParam)
				{
					auto pThis = (CVeListTemplate*)lParam;
					pThis->ReCalcTopItem();
			...
			m_psvH->AddRef();
			m_psvH->SetMinThumbSize(eck::Dui::CxyMinScrollThumb);
	...
	int HitTest(LE_HITTEST& leht) const
	{
		...
		if (!eck::PtInRect(GetViewRectF(), leht.pt))
			return -1;
	...
	{
		NMLTITEMCHANGE nm{ eck::Dui::LTE_ITEMCHANED };
			...
			else
			{
				...
				if (idx0 >= 0)
				{
					...
					else
					{
						...
						D2D1_RECT_F rc;
						GetItemRect(idx1, rc);
						eck::UnionRect(rcInvalid, rcInvalid, rc);
					}
				}
				else
					rcInvalid = {};
	...
	{
		NMLTITEMINDEX nm{ eck::Dui::EE_CLICK };
		nm.idx = idx;
		nm.idxGroup = idxGroup;
		GenElemNotify(&nm);
		if (m_bItemNotify)
		{
			NMLTITEMCHANGE nm2{ eck::Dui::LTE_ITEMCHANED };
			nm2.uFlagsOld = 0;
	...
	void SetImageList(eck::CD2DImageList* pImgList)
	{
		ECK_DUILOCK;
	...
	void SetGroupImageList(eck::CD2DImageList* pImgList)
	{
		ECK_DUILOCK;
	...
	void SetHeaderItemWidth(int idx, float width) {
		m_Header.SetHeaderItemWidth(idx, width);
	}
