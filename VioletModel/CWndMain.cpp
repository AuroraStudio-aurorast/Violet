...
void CWndMain::ClearRes()
{
	...
	SafeRelease(m_pCompPageTransAn);
...
BOOL CWndMain::OnCreate(HWND hWnd, CREATESTRUCT* pcs)
{
	...
	std::wstring b;
	std::wstring c;
	a = (std::wstring)CurrentDirectory;
	b = (std::wstring)L"\\lang\\zh-cn.lang";
	c = a + b;
	m_AppLang = readLang(c.c_str());

	MainWndPageName[0] = m_AppLang.LANG_ID_STARTUP_TITLE;
	MainWndPageName[1] = m_AppLang.LANG_ID_PLAYLIST_TITLE;
	MainWndPageName[2] = m_AppLang.LANG_ID_EFFECT_TITLE;
	MainWndPageName[3] = m_AppLang.LANG_ID_SETTINGS_TITLE;

	App->GetListMgr().Add()->SetName(m_AppLang.LANG_ID_PLAYLIST_PLAYLIST);
	App->GetListMgr().Add()->SetName(L"2"sv);
	...
	m_pCompPageTransAn = new Dui::CCompositorPageAn{};
	m_pCompPageTransAn->SetWorkDC(GetDeviceContext());
	m_pCompPageTransAn->InitAsScaleOpacity();
	...
	// 底部播放控制栏
	m_PlayPanel.Create(nullptr, Dui::DES_VISIBLE | Dui::DES_CONTENT_EXPAND, 0,
		0, 0, 0, 0, nullptr, this);
	...
	App->GetFontFactory().NewFont(pTfPP.RefOfClear(), eck::Align::Near,
		eck::Align::Center, (float)CyFontPlayPageTitle, 600);
	...
	m_TitleBar.Create(nullptr, Dui::DES_VISIBLE, 0,
		0, 0, 0, 0, nullptr, this);
	m_TitleBar.SetInterpolationMode(D2D1_INTERPOLATION_MODE_HIGH_QUALITY_CUBIC);
	// 音量条
	m_VolBar.Create(nullptr, Dui::DES_CONTENT_EXPAND, 0,
			...
			m_pecPage->SetCallBack([](float fCurrValue, float fOldValue, LPARAM lParam)
				{
					...
					constexpr float yNormal = 0;//CyPageTitle + DTopPageTitle + CxPageIntPadding;
					//p->m_pAnPage->SetCompositor(nullptr);

					float opacity = fCurrValue / CyPageSwitchAnDelta;
					float scale = 0.8 + (fCurrValue / CyPageSwitchAnDelta) * 0.2;// * (1-opacity) * 0.2;
					p->m_pCompPageTransAn->Scale = scale;
					p->m_pCompPageTransAn->Update2DAffineTransform();
					p->m_pCompPageTransAn->SetBlurStdDeviation(40.f * (1 - scale));

					p->m_pCompPageTransAn->Opacity = opacity;
					p->m_pCompPageTransAn->UpdateOpacity();
					p->m_pAnPage->SetStyle(Dui::DES_BASE_BEGIN_END_PAINT | p->m_pAnPage->GetStyle());

					//p->m_pAnPage->SetCompositor(nullptr);
					if (!p->m_pAnPage->GetCompositor()) {
						p->m_pAnPage->SetCompositor(p->m_pCompPageTransAn);
					...
					//p->m_pAnPage->CompUpdateCacheBitmap(p->m_pAnPage->GetWidthF(), p->m_pAnPage->GetWidthF());
					//p->m_pAnPage->CompTransformCoordinate();
					
					p->m_pAnPage->CompReCalcCompositedRect();
					if (p->m_pecPage->IsStop()) {
						p->m_pAnPage->SetCompositor(nullptr);
					}
...
void CWndMain::ClearPageAnimation()
{
	if (!m_pAnPage)
		return;
	m_pecPage->End();
	m_pAnPage->SetCompositor(nullptr);
	m_pAnPage = nullptr;
}
...
LRESULT CWndMain::OnMsg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		m_pCompPageTransAn->RefPoint = { cxClient / 2.f,cyClient / 2.f };
		m_NormalPageContainer.SetRect({ 0,0,cxClient,cyClient });
		m_TitleBar.SetRect({ 0,0,cxClient,CyTitleBar });
		m_TabPanel.SetRect({ 0,0,CxTabPanel,cyClient });

		const auto yPlayPanel = cyClient - CyPlayPanel;
		//m_PlayPanel.SetRect({ 0,cyClient - CyPlayPanel,cxClient,cyClient });
		m_PlayPanel.SetRect({ CxyPlayPanelPadding,cyClient - CyPlayPanel - CxyPlayPanelPadding,cxClient - CxyPlayPanelPadding,cyClient - CxyPlayPanelPadding });
		...
		m_rcPPMini.left = DLeftMiniCover + CxyPlayPanelPadding;
		m_rcPPMini.top = float(cyClient - CyPlayPanel + DTopMiniCover - CxyPlayPanelPadding);
		m_rcPPMini.right = m_rcPPMini.left + (float)CxyMiniCover;
		m_rcPPMini.bottom = m_rcPPMini.top + (float)CxyMiniCover;
		for (auto& e : m_vPage)
			e->SetRect({
				CxTabPanel + CxTabToPagePadding,
				0,//CyPageTitle + DTopPageTitle + CxPageIntPadding,
				cxClient,
	...
	case WM_DWMCOLORIZATIONCOLORCHANGED:
		StUpdateColorizationColor();
		//D2D1_COLOR_F colorizationColor;
		//GetStdTheme()->GetColorizationColor(colorizationColor);
		//highlightColor = GDIClrToCommonClr(RGB(colorizationColor.r, colorizationColor.g, colorizationColor.b));
		break;
	case WM_SETTEXT:
	{
		const auto lResult = __super::OnMsg(hWnd, uMsg, wParam, lParam);
...
LRESULT CWndMain::OnElemEvent(Dui::CElem* pElem, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case Dui::TBE_POSCHANGED:
	{
		...
		{
			const auto f = ((CVeTrackBar*)pElem)->GetTrackPos();
			App->GetPlayer().SetVolume(f / 100.f);
...
void CWndMain::TlTick(int iMs)
{
	...
	constexpr float DurOverlayOpacity = 120.f;
	...
	m_PlayPanel.setFade(1 - m_PlayPageAn.K);
	// 页面动画更新
	const auto kOverlay = std::clamp(m_PlayPageAn.Time / DurOverlayOpacity, 0.f, 1.f);
	m_pCompPlayPageAn->Opacity = 0.f;// (m_bPPAnReverse ? (1.f - kOverlay) : kOverlay);
	m_PagePlaying.setCoverImgOpacity((m_bPPAnReverse ? (1.f - kOverlay) : kOverlay));
	...
	if (!m_PagePlaying.GetCompositor())
		m_PagePlaying.SetCompositor(m_pCompPlayPageAn, false);
	...
	float ratioX = (m_rcPPLarge.right - m_rcPPLarge.left) / (m_rcPPMini.right - m_rcPPMini.left);
	float ratioY = (m_rcPPLarge.bottom - m_rcPPLarge.top) / (m_rcPPMini.bottom - m_rcPPMini.top);
	float ratioXY = ratioX / ratioY;
	float ratioX_ = ratioX * (1 + m_PlayPageAn.K / ratioXY);
	float ratioY_ = ratioY * (1 + m_PlayPageAn.K * ratioXY);
	float smallCoverRadius = 10;

	float radiusX_ = smallCoverRadius * ratioX_ * (1 - m_PlayPageAn.K);
	float radiusY_ = smallCoverRadius * ratioY_ * (1 - m_PlayPageAn.K);
	m_PagePlaying.setRadiusXY(radiusX_, radiusY_);
...
void CWndMain::UpdateButtonImageSize()
{
	...
	m_BTAutoNext.SetImageSize(Size);
}
...
void CWndMain::RePosButtonProgBar()
{
	...
	const auto cxClient = GetClientWidthLog();
	const auto cyClient = GetClientHeightLog();
	float btnYOffset = CxyPlayPanelPadding * (1 - m_PlayPageAn.K);
	// 移动右侧按钮
	const auto y = cyClient - CyPlayPanel + (CyPlayPanel - CxyCircleButton) / 2 - btnYOffset;
	...
	m_BTPrev.SetPos(x, y);
	x += (CxyCircleButton + CxPaddingCircleButton);
	m_BTPlay.SetPos(x, cyClient - CyPlayPanel + (CyPlayPanel - CxyCircleButtonBig) / 2 - btnYOffset);
	x += (CxyCircleButtonBig + CxPaddingCircleButton);
	m_BTNext.SetPos(x, y);
	// 移动进度条
	const auto yPlayPanel = cyClient - CyPlayPanel - CxyPlayPanelPadding;
	const auto dTrackSpacing = m_TBProgress.GetTrackSpacing();
	const auto oxIndent = (float)CxPaddingProgBarWithPlayPage * m_PlayPageAn.K + CxyPlayPanelPadding;
	float progYOffset = (CxyPlayPanelPadding - 3) * (1 - m_PlayPageAn.K) - CxyPlayPanelPadding;
	m_TBProgress.SetRect({
		-dTrackSpacing + oxIndent,
		yPlayPanel - CyProgress / 2 - progYOffset,
		cxClient + dTrackSpacing - oxIndent,
		yPlayPanel + CyProgress / 2 - progYOffset });
	if (m_PlayPageAn.K > 0.6) {
		m_TBProgress.setFixedTop(false);
		m_TBProgress.setUseHighlightColor(false);
		m_TBProgress.setRoundedRectMaskEnabled(false);
	}
	else {
		m_TBProgress.setFixedTop(true);
		m_TBProgress.setUseHighlightColor(true);
		D2D1_RECT_F TBRect = m_TBProgress.GetRectF();
		//m_TBProgress.setRoundedRectMask({ 0,0,TBRect.right - TBRect.bottom,CyPlayPanel },10);
		m_TBProgress.setRoundedRectMask({ CxyPlayPanelPadding - TBRect.left + 1, cyClient - CyPlayPanel - CxyPlayPanelPadding - TBRect.top, cxClient - CxyPlayPanelPadding - TBRect.left - 1, cyClient - CxyPlayPanelPadding - TBRect.top }, 10);

		m_TBProgress.setRoundedRectMaskEnabled(true);
	}
